...
import { getFinancialReport } from "../graphql/queries";
import { updateFinancialReport } from "../graphql/mutations";
const client = generateClient();
export default function FinancialReportUpdateForm(props) {
  const {
    id: idProp,
    financialReport: financialReportModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = financialReportRecord
      ? { ...initialValues, ...financialReportRecord }
      : initialValues;
    setReportName(cleanValues.reportName);
    setReportType(cleanValues.reportType);
    setReportDate(cleanValues.reportDate);
    setStartDate(cleanValues.startDate);
    setEndDate(cleanValues.endDate);
    setReportData(
      typeof cleanValues.reportData === "string" ||
        cleanValues.reportData === null
        ? cleanValues.reportData
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getFinancialReport.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getFinancialReport
        : financialReportModelProp;
      setFinancialReportRecord(record);
    };
    queryData();
  }, [idProp, financialReportModelProp]);
  React.useEffect(resetStateValues, [financialReportRecord]);
  const validations = {
    reportName: [],
    reportType: [],
    reportDate: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateFinancialReport.replaceAll("__typename", ""),
            variables: {
              input: {
                id: financialReportRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "FinancialReportUpdateForm")}
      {...rest}
    >
      <TextField
        label="Report name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || financialReportModelProp)}
