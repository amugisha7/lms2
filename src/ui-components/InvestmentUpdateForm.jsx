...
import { getInvestment } from "../graphql/queries";
import { updateInvestment } from "../graphql/mutations";
const client = generateClient();
export default function InvestmentUpdateForm(props) {
  const {
    id: idProp,
    investment: investmentModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = investmentRecord
      ? { ...initialValues, ...investmentRecord }
      : initialValues;
    ...
    setInvestmentStatus(cleanValues.investmentStatus);
    setInvestmentAttribute1(cleanValues.investmentAttribute1);
    setInvestmentAttribute2(cleanValues.investmentAttribute2);
    setNumberOfPayments(cleanValues.numberOfPayments);
    setPaymentFrequency(cleanValues.paymentFrequency);
    setStatus(cleanValues.status);
    setCustomInvestmentDetails(
      typeof cleanValues.customInvestmentDetails === "string" ||
        cleanValues.customInvestmentDetails === null
        ? cleanValues.customInvestmentDetails
        : JSON.stringify(cleanValues.customInvestmentDetails)
    );
    setErrors({});
  };
  const [investmentRecord, setInvestmentRecord] =
    React.useState(investmentModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getInvestment.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getInvestment
        : investmentModelProp;
      setInvestmentRecord(record);
    };
    queryData();
  }, [idProp, investmentModelProp]);
  React.useEffect(resetStateValues, [investmentRecord]);
  const validations = {
    principal: [],
    description: [],
    fees: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          investmentAttribute1: investmentAttribute1 ?? null,
          investmentAttribute2: investmentAttribute2 ?? null,
          numberOfPayments: numberOfPayments ?? null,
          paymentFrequency: paymentFrequency ?? null,
          status: status ?? null,
          customInvestmentDetails: customInvestmentDetails ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateInvestment.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "InvestmentUpdateForm")}
      {...rest}
    >
      <TextField
        label="Principal"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || investmentModelProp)}
