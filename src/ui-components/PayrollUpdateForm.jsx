...
import { updatePayroll } from "../graphql/mutations";
const client = generateClient();
export default function PayrollUpdateForm(props) {
  const {
    id: idProp,
    payroll: payrollModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = payrollRecord
      ? { ...initialValues, ...payrollRecord }
      : initialValues;
    setPeriodStartDate(cleanValues.periodStartDate);
    setPeriodEndDate(cleanValues.periodEndDate);
    setPayDate(cleanValues.payDate);
    setStatus(cleanValues.status);
    setProcessedByUserID(cleanValues.processedByUserID);
    setTotalGrossPay(cleanValues.totalGrossPay);
    setTotalLoanDeductions(cleanValues.totalLoanDeductions);
    setTotalSavingsDeductions(cleanValues.totalSavingsDeductions);
  ...
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getPayroll.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPayroll
        : payrollModelProp;
    ...
    queryData();
  }, [idProp, payrollModelProp]);
  React.useEffect(resetStateValues, [payrollRecord]);
  const validations = {
    periodStartDate: [],
    periodEndDate: [],
    payDate: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          totalGrossPay: totalGrossPay ?? null,
          totalLoanDeductions: totalLoanDeductions ?? null,
          totalSavingsDeductions: totalSavingsDeductions ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updatePayroll.replaceAll("__typename", ""),
            variables: {
              input: {
                id: payrollRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "PayrollUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || payrollModelProp)}
