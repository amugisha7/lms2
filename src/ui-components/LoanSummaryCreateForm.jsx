...
import { createLoanSummary } from "../graphql/mutations";
const client = generateClient();
export default function LoanSummaryCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setLoanID(initialValues.loanID);
    setRefreshScope(initialValues.refreshScope);
    setInstitutionID(initialValues.institutionID);
    setBranchID(initialValues.branchID);
    setBorrowerID(initialValues.borrowerID);
    setBorrowerDisplayName(initialValues.borrowerDisplayName);
    setBorrowerDisplayNameNormalized(
      initialValues.borrowerDisplayNameNormalized
    );
    setBorrowerPhone(initialValues.borrowerPhone);
    setLoanNumber(initialValues.loanNumber);
    setLoanOfficerID(initialValues.loanOfficerID);
    setLoanOfficerDisplayName(initialValues.loanOfficerDisplayName);
    setLoanProductID(initialValues.loanProductID);
    setLoanProductName(initialValues.loanProductName);
    setPrincipalAmount(initialValues.principalAmount);
    setTotalPaidAmount(initialValues.totalPaidAmount);
    setAmountDueAmount(initialValues.amountDueAmount);
    setLoanBalanceAmount(initialValues.loanBalanceAmount);
    setArrearsAmount(initialValues.arrearsAmount);
    setMissedInstallmentCount(initialValues.missedInstallmentCount);
    setNextDueDate(initialValues.nextDueDate);
    setLastPaymentDate(initialValues.lastPaymentDate);
    setStartDate(initialValues.startDate);
    setMaturityDateEffective(initialValues.maturityDateEffective);
    setLifecycleStatus(initialValues.lifecycleStatus);
    setDisplayStatus(initialValues.displayStatus);
    setDisplayStatusRank(initialValues.displayStatusRank);
    setDisplayStatusComputedAt(initialValues.displayStatusComputedAt);
    setNextStatusTransitionAt(initialValues.nextStatusTransitionAt);
    setCurrencyCode(initialValues.currencyCode);
    setErrors({});
  };
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createLoanSummary.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "LoanSummaryCreateForm")}
      {...rest}
    >
      <TextField
        label="Loan id"
