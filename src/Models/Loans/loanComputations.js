...
/**
 * Pure computation: takes the current (possibly unsaved) loan draft form values
 * and returns a schedule preview compatible with existing exports.
 *
 * No API calls.
 */
	...
	const maturityDate =
		...
		(startDate && duration && durationInterval
			? dayjs(startDate)
					.add(
						duration,
						durationUnitMap[durationInterval] || durationInterval
					)
					.format("YYYY-MM-DD")
			: null);
	...
	if (interestCalculationMethod === "FLAT") {
		...
		if (interestType === "fixed") {
			if (interestPeriod === "per_loan") {
				...
				totalInterest = rawInterestRate *
					(interestPeriod === "per_year" ? durationInYears : 1);
