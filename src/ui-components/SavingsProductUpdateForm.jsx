...
import { getSavingsProduct } from "../graphql/queries";
import { updateSavingsProduct } from "../graphql/mutations";
const client = generateClient();
export default function SavingsProductUpdateForm(props) {
  const {
    id: idProp,
    savingsProduct: savingsProductModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = savingsProductRecord
      ? { ...initialValues, ...savingsProductRecord }
      : initialValues;
    setName(cleanValues.name);
    setType(cleanValues.type);
    setInterestRate(cleanValues.interestRate);
    setInterestPostingFrequency(cleanValues.interestPostingFrequency);
    setMinBalance(cleanValues.minBalance);
    setCustomSavingsProductDetails(
      typeof cleanValues.customSavingsProductDetails === "string" ||
        cleanValues.customSavingsProductDetails === null
        ? cleanValues.customSavingsProductDetails
        : JSON.stringify(cleanValues.customSavingsProductDetails)
    );
    setErrors({});
  };
  const [savingsProductRecord, setSavingsProductRecord] = React.useState(
    savingsProductModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getSavingsProduct.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getSavingsProduct
        : savingsProductModelProp;
      setSavingsProductRecord(record);
    };
    queryData();
  }, [idProp, savingsProductModelProp]);
  React.useEffect(resetStateValues, [savingsProductRecord]);
  const validations = {
    name: [],
    type: [],
    interestRate: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          customSavingsProductDetails: customSavingsProductDetails ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateSavingsProduct.replaceAll("__typename", ""),
            variables: {
              input: {
                id: savingsProductRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "SavingsProductUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      <TextAreaField
        ...
        isRequired={false}
        isReadOnly={false}
        value={customSavingsProductDetails}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || savingsProductModelProp)}
