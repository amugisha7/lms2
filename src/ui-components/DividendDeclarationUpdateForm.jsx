...
import { getDividendDeclaration } from "../graphql/queries";
import { updateDividendDeclaration } from "../graphql/mutations";
const client = generateClient();
export default function DividendDeclarationUpdateForm(props) {
  const {
    id: idProp,
    dividendDeclaration: dividendDeclarationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = dividendDeclarationRecord
      ? { ...initialValues, ...dividendDeclarationRecord }
      : initialValues;
    setFiscalYear(cleanValues.fiscalYear);
    setDistributableSurplus(cleanValues.distributableSurplus);
    setDividendRate(cleanValues.dividendRate);
    setInterestRebateRate(cleanValues.interestRebateRate);
    setStatus(cleanValues.status);
    setCustomDividendDeclarationDetails(
      typeof cleanValues.customDividendDeclarationDetails === "string" ||
        cleanValues.customDividendDeclarationDetails === null
        ? cleanValues.customDividendDeclarationDetails
        : JSON.stringify(cleanValues.customDividendDeclarationDetails)
    );
    setErrors({});
  };
  const [dividendDeclarationRecord, setDividendDeclarationRecord] =
    React.useState(dividendDeclarationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getDividendDeclaration.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getDividendDeclaration
        : dividendDeclarationModelProp;
      setDividendDeclarationRecord(record);
    };
    queryData();
  }, [idProp, dividendDeclarationModelProp]);
  React.useEffect(resetStateValues, [dividendDeclarationRecord]);
  const validations = {
    fiscalYear: [],
    distributableSurplus: [],
    dividendRate: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          distributableSurplus: distributableSurplus ?? null,
          dividendRate: dividendRate ?? null,
          interestRebateRate: interestRebateRate ?? null,
          status: status ?? null,
          customDividendDeclarationDetails:
            customDividendDeclarationDetails ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateDividendDeclaration.replaceAll("__typename", ""),
            variables: {
              input: {
                id: dividendDeclarationRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "DividendDeclarationUpdateForm")}
      {...rest}
    >
      <TextField
        label="Fiscal year"
      ...
      <TextAreaField
        ...
        isRequired={false}
        isReadOnly={false}
        value={customDividendDeclarationDetails}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || dividendDeclarationModelProp)}
